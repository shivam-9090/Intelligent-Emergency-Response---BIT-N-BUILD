...
def main() -> None:
    ...
    missingness = (
        {field: round(1 - sum(present(row.get(field)) for row in rows) / len(rows), 4) for field in fields}
        if rows
        else {}
    )
    ...
    geocoded = (
        sum(present(row.get(args.lat_field)) and present(row.get(args.lon_field)) for row in rows)
        if args.lat_field and args.lon_field
        else 0
    )
    report = {
        "source": args.source,
